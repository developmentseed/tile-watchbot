...
def process(message):
    ...
    # mosaic+https://
    # mosaic+s3://
    parsed = urlparse(message.dataset)
    if parsed.scheme and parsed.scheme.startswith("mosaic+"):
        mosaic_dataset = message.dataset.replace("mosaic+", "")
    ...
    # BaseReader
        ...
        bname = os.path.basename(message.dataset).split(".")[0]
        out_key = os.path.join(
            bname, f"{message.tile.x}-{message.tile.y}-{message.tile.z}.npy",
        )
        with reader(message.dataset) as src_dst:
            if not message.expression:
                bidx_kwargs = _get_options(src_dst, message.indexes)
                kwargs = {**kwargs, **bidx_kwargs}
    ...
    return _s3_upload(
        BytesIO(data.render(img_format="npy")),
        out_bucket,
        out_key,
    )
